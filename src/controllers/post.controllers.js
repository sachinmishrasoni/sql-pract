...
export const updatePostById = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query("UPDATE posts SET title = ?, content = ? WHERE id = ?", [title, content, postId]);
...
export const getPostsByUserId = async (req, res) => {
    try {
        const { userId } = req.params;
        const [rows] = await pool.query("SELECT * FROM posts WHERE user_id = ?", [userId]);
        res.status(200).json({
            status: true,
            results: rows.length,
            data: rows
...
export const getPostsByUserIdAndPostId = async (req, res) => {
    try {
        const { userId, postId } = req.params;
        const [rows] = await pool.query("SELECT * FROM posts WHERE user_id = ? AND id = ?", [userId, postId]);
        res.status(200).json({
            status: true,
            results: rows.length,
            data: rows
